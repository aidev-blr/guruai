...
def get_subjects():
    try:
        ...
        # Get class level from user data with fallback to request args
        class_level = request.args.get('class') or user_data.get('class', '')
        stream = request.args.get('stream') or user_data.get('stream', 'NA')
...
# Subjects by class level and stream
        if class_level <= 10:
            subjects = core_subjects + ['Maths', 'Science', 'Social Science']
            
            # Additional subjects for class 9-10
            if class_level >= 9:
                subjects += ['Computer Science', 'Hindi']
        else:
            # Senior secondary subjects by stream
            if stream == 'Science':
                ...
                # Default subjects if stream not specified
                subjects = core_subjects + [
                    'Maths',
                    'Computer Science',
                    'Economics',
...
def generate_quiz():
    try:
        ...
        # Force NCERT board regardless of user's board setting
        board = "NCERT"
        ...
        try:
            # Attempt to clean the response text before parsing
            json_text = response.text.strip()
            if json_text.startswith("```json"):
                json_text = json_text[len("```json"):].strip()  # Remove ```json
            if json_text.endswith("```"):
                json_text = json_text[:-len("```")].strip()  # Remove ``` (if present)

            questions = json.loads(json_text)

        except json.JSONDecodeError:
            app.logger.error(f"Invalid JSON format from model: {response.text}")
